    ...
    def forward(
        ...
        for index_block, block in enumerate(self.transformer_blocks):
            if self.training and self.gradient_checkpointing:
                ...
                # ==================================
                # here should be a PuLID replace
                if id_embeddings is not None and index_block % self.pulid_double_interval == 0:
                    for id_weight, id_embedding, HB_m_offset, HB_n_offset, HB_m_scale,HB_n_scale in zip(id_weights, id_embeddings, HB_m_offset_list, HB_n_offset_list, HB_m_scale_list, HB_n_scale_list):
                        id_mask = torch.zeros_like(hidden_states).view(hidden_states.shape[0], latent_h, latent_w, hidden_states.shape[2])
                        id_mask[:, HB_n_offset:HB_n_offset+HB_n_scale,HB_m_offset:HB_m_offset+HB_m_scale, :] = 1.0
                        id_mask = id_mask.view(hidden_states.shape[0], latent_h*latent_w, hidden_states.shape[2])
