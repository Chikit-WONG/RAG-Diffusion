...
class IPAFluxAttnProcessor2_0(nn.Module):
    ...
    def __init__(self, hidden_size=3072, cross_attention_dim=4096, scale_list=None, num_tokens=4):
        super().__init__()
    ...
    def __call__(
        ...
        if image_emb_list is not None:
            for image_emb in image_emb_list:
        ...
        if encoder_hidden_states is not None:
            ...
            if image_emb_list is not None:
                if image_mask_list is not None:
                    for ip_hidden_states, mask, scale in zip(ip_hidden_states_list, image_mask_list, self.scale_list):
                        mask = mask.repeat(hidden_states.shape[0], 1, hidden_states.shape[-1]).to(hidden_states.dtype)
                        hidden_states = hidden_states + scale * ip_hidden_states * mask
                else:
                    for ip_hidden_states, scale in zip(ip_hidden_states_list, self.scale_list):
            ...
            if image_emb_list is not None:
                if image_mask_list is not None:
                    for ip_hidden_states, mask, scale in zip(ip_hidden_states_list, image_mask_list, self.scale_list):
                        empty_mask = torch.zeros([1, 512, 1], dtype=torch.bool).to(hidden_states.device)
                        mask = torch.cat([empty_mask, mask], dim=1)
                        mask = mask.repeat(hidden_states.shape[0], 1, hidden_states.shape[-1]).to(hidden_states.dtype)
                        hidden_states = hidden_states + scale * ip_hidden_states * mask
                else:
                    for ip_hidden_states, scale in zip(ip_hidden_states_list, self.scale_list):
