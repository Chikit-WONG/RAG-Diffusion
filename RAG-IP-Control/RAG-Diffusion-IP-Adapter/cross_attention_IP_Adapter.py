...
def FluxSingleTransformerBlock_hook_forward(self, module):
    def forward(hidden_states=None, encoder_hidden_states=None, image_rotary_emb=None, SR_encoder_hidden_states_list=None, SR_norm_encoder_hidden_states_list=None, SR_hidden_states_list=None, SR_norm_hidden_states_list=None, image_emb_list=None, image_mask_list=None):
        flux_hidden_states=module.processor(module, hidden_states=hidden_states, image_rotary_emb=image_rotary_emb, image_emb_list=image_emb_list, image_mask_list=image_mask_list)
        ...
        def single_matsepcalc(x, contexts_list, image_rotary_emb):
            ...
            for drow in self.split_ratio:
                ...
                for dcell in drow.cols:
                    ...
                    image_emb = image_emb_list[i]
                    i = i + 1 + dcell.breaks
                    SR_all_out = module.processor(module, hidden_states=context, image_rotary_emb=image_rotary_emb, image_emb_list=[image_emb])
                    out = SR_all_out[:, 512 :, ...]
...
def FluxTransformerBlock_hook_forward(self, module):
    def forward(hidden_states=None, encoder_hidden_states=None, image_rotary_emb=None, SR_encoder_hidden_states_list=None, SR_norm_encoder_hidden_states_list=None, SR_hidden_states_list=None, SR_norm_hidden_states_list=None, image_emb_list=None, image_mask_list=None):
        flux_hidden_states, flux_encoder_hidden_states = module.processor(module, hidden_states=hidden_states, encoder_hidden_states=encoder_hidden_states, image_rotary_emb=image_rotary_emb, image_emb_list=image_emb_list, image_mask_list=image_mask_list)
        ...
        def matsepcalc(x, contexts_list, image_rotary_emb):
            ...
            for drow in self.split_ratio:
                ...
                for dcell in drow.cols:
                    context = contexts_list[i]
                    imgae_emb = image_emb_list[i]
                    i = i + 1 + dcell.breaks
                    out,SR_context_attn_output = module.processor(module, hidden_states=x, encoder_hidden_states=context, image_rotary_emb=image_rotary_emb, image_emb_list=[imgae_emb])
                    out = out.reshape(out.size()[0], latent_h, latent_w, out.size()[2]) 
                    addout = 0
                    addin = 0
                    sumin = sumin + int(latent_in*dcell.end) - int(latent_in*dcell.start)
...
def FluxSingleTransformerBlock_init_forward(self, module):
    def forward(hidden_states=None, encoder_hidden_states=None, image_rotary_emb=None,RPG_encoder_hidden_states_list=None,RPG_norm_encoder_hidden_states_list=None,RPG_hidden_states_list=None,RPG_norm_hidden_states_list=None, image_emb_list=None):
        return module.processor(module, hidden_states=hidden_states, image_rotary_emb=image_rotary_emb, image_emb_list=image_emb_list)
    return forward

def FluxTransformerBlock_init_forward(self, module):
    def forward(hidden_states=None, encoder_hidden_states=None, image_rotary_emb=None,RPG_encoder_hidden_states_list=None,RPG_norm_encoder_hidden_states_list=None,RPG_hidden_states_list=None,RPG_norm_hidden_states_list=None, image_emb_list=None):
        return module.processor(module, hidden_states=hidden_states, encoder_hidden_states=encoder_hidden_states, image_rotary_emb=image_rotary_emb, image_emb_list=image_emb_list)
    return forward